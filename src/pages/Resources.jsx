...
const Resources = () => {
    ...
    // Fetch Gemini AI summary for a topic
    const fetchGeminiSummary = async (topic) => {
        if (!topic || geminiSummaries[topic]) return; // Already have summary
        
        setLoadingGemini(prev => ({ ...prev, [topic]: true }));
        
        try {
            const token = localStorage.getItem('jwt_token');
            const response = await fetch(`${API_BASE_URL}/topic-summary/${encodeURIComponent(topic)}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            
            if (response.ok) {
                const data = await response.json();
                setGeminiSummaries(prev => ({ ...prev, [topic]: data.summary }));
                console.log(`Gemini summary fetched for ${topic}:`, data);
            } else {
                console.error('Failed to fetch Gemini summary:', await response.text());
            }
        } catch (error) {
            console.error('Error fetching Gemini summary:', error);
        } finally {
            setLoadingGemini(prev => ({ ...prev, [topic]: false }));
        }
    };

    // Download topic summary (Gemini + Video info)
    const downloadTopicSummary = (topic, video) => {
        const geminiSummary = geminiSummaries[topic] || '';
...
🤖 AI-GENERATED TOPIC SUMMARY (Powered by Gemini):
----------------------------------------
${geminiSummary}
...
========================================`;
        ...
        a.href = url;
        a.download = `${topic.replace(/[^a-zA-Z0-9]/g, '_')}_Complete_Summary.txt`;
    ...
    useEffect(() => {
        if (recommendations.length > 0) {
            ...
            const fetchAllVideos = async () => {
                ...
                // Fetch Gemini summaries for all topics
                for (const rec of recommendations) {
                    const topic = rec.topic || rec.gate_topic;
                    if (topic && !geminiSummaries[topic]) {
                        fetchGeminiSummary(topic);
                    }
                }
            };
                    ...
                    {tod["Best Video"] && (
                        <div style={{ background: 'linear-gradient(135deg, #1a1a2e, #16213e)', borderRadius: '12px', overflow: 'hidden', color: '#fff' }}>
                            ...
                            <div style={{ padding: '1.2rem' }}>
                                ...
                                <button
                                    onClick={() => downloadTopicSummary(tod["Topic Name"] || tod["Topic"], tod["Best Video"])}
                                    style={{
                                        display: 'inline-flex', alignItems: 'center', gap: '8px',
                                ...
                                <div style={{
                                    ...
                                    {geminiSummaries[topic] && (
                                        <div style={{ 
                                            ...
                                            <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem', color: '#3498db' }}>
                                                <i className="fa-solid fa-robot" style={{ marginRight: '6px' }}></i>
                                                🤖 AI Topic Summary
                                            </h4>
                                            <p style={{ margin: 0, fontSize: '0.85rem', color: '#ecf0f1', lineHeight: 1.5 }}>
                                                {geminiSummaries[topic].substring(0, 200)}...
                                            </p>
                                        </div>
                                    )}
                                    {loadingGemini[topic] && (
                                        <div style={{ marginTop: '1rem', fontSize: '0.9rem', color: '#bdc3c7' }}>
                                        ...
                                        {geminiSummaries[topic] && (
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                                                <div>
                                                    ...
                                                    <p style={{ margin: 0, fontSize: '0.9rem', color: '#e8daef' }}>
                                                        Get AI-generated topic summary + video notes
                                                    </p>
                                                </div>
                                                <button
                                                    onClick={() => downloadTopicSummary(topic, videos[0])}
                                                    style={{
                                                        background: '#fff',
                                                        color: '#9b59b6',
                                                        fontWeight: 700,
                                                ...
                                                >
                                                    <i className="fa-solid fa-file-arrow-down" style={{ marginRight: '6px' }}></i>
                                                    Download
                                                </button>
                                            </div>
                                        )}
